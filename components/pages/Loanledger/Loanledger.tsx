...
const LoanLedgerPDFPage = ({ formData }: { formData: LoanFormData }) => {
  ...
  const nonEmptyRows = formData.tableRows.filter(row => 
    row.cropName || 
    row.extentLand || 
    row.cash || 
    row.seedRawMaterials || 
    row.fertilizer || 
    row.microNutrients || 
    row.pesticide || 
    row.othersInsurance || 
    ...
    <div className="p-12 bg-white min-h-[2500px] min-w-[1800px] text-black font-serif text-[10px] leading-tight" 
      ...
      <div className="grid grid-cols-3 gap-x-4 gap-y-1 mb-4 relative">
        ...
        <div className="flex items-baseline">
          ...
          <span className="font-bold text-sm">{formData.uEnn}</span>
        </div>
        <div className="flex items-baseline">
            ...
            {nonEmptyRows.length > 0 ? (
              <>
                {nonEmptyRows.map((row, rowIndex) => (
                  <tr key={rowIndex}>
                    <td className="border border-black p-1 text-center text-[10px]">{rowIndex + 1}</td>
                    ...
                    {rowIndex > 1 && (
                      <td className="border border-black p-1 text-left text-[10px]"></td>
                    )}
                    <td className="border border-black p-1 text-right text-[10px]"></td>
                    <td className="border border-black p-1 text-right text-[10px]">
                      {rowIndex === 0
                        ? nonEmptyRows.reduce(
                            (sum, row) => sum + ((Number(row.cash) || 0) - (Number(row.fertilizer) || 0)),
                            0
                          )
                        : rowIndex === 1
                        ? nonEmptyRows.reduce(
                            (sum, row) => sum + (Number(row.fertilizer) || 0),
                            0
                          )
                        : ""}
                    </td>
                ...
                <tr>
                  ...
                  <td className="border border-black p-1 text-center font-bold text-[10px]">
                    {nonEmptyRows.reduce((sum, row) => {
                      // Convert to number and handle potential non-numeric values
                      const acreValue = parseFloat(row.extentLand?.toString() || "0");
                      return sum + (isNaN(acreValue) ? 0 : acreValue);
                    }, 0)}
                  </td>
                  <td className="border border-black p-1 text-right font-bold text-[10px]">
                    {nonEmptyRows.reduce((sum, row) => sum + (Number(row.cash) || 0), 0)}
                  </td>
                  <td className="border border-black p-1 text-right font-bold text-[10px]">
                    {nonEmptyRows.reduce((sum, row) => sum + (Number(row.seedRawMaterials) || 0), 0)}
                  </td>
                  <td className="border border-black p-1 text-right font-bold text-[10px]">
                    {nonEmptyRows.reduce((sum, row) => sum + (Number(row.fertilizer) || 0), 0)}
                  </td>
                  <td className="border border-black p-1 text-right font-bold text-[10px]">
                    {nonEmptyRows.reduce((sum, row) => sum + (Number(row.microNutrients) || 0), 0)}
                  </td>
                  <td className="border border-black p-1 text-right font-bold text-[10px]">
                    {nonEmptyRows.reduce((sum, row) => sum + (Number(row.pesticide) || 0), 0)}
                  </td>
                  <td className="border border-black p-1 text-right font-bold text-[10px]">
                    {nonEmptyRows.reduce((sum, row) => sum + (Number(row.othersInsurance) || 0), 0)}
                  </td>
                  <td className="border border-black p-1 text-right font-bold text-[10px]">
                    {nonEmptyRows.reduce((sum, row) => sum + (Number(row.total) || 0), 0)}
                  </td>
                  <td className="border border-black p-1"></td>
                  <td className="border border-black p-1"></td>
                  <td className="border border-black p-1 text-right font-bold text-[10px]"></td>
                  <td className="border border-black p-1 text-right font-bold text-[10px]"></td>
                  <td className="border border-black p-1 text-right font-bold text-[10px]"></td>
                  <td className="border border-black p-1 text-right font-bold text-[10px]"></td>
                  <td className="border border-black p-1 text-right font-bold text-[10px]"></td>
              ...
              <tr>
                <td colSpan={21} className="border border-black p-2 text-center text-[10px]">
                  No data available
                </td>
              </tr>
            )}
      ...
      members: [
        {
          userInformation: {
            ...
            user_photo_preview: "/placeholder.svg?height=96&width=80",
          },
          documents: {
            userPhoto: { preview: "/placeholder.svg?height=96&width=80" },
            friendPhoto: { preview: "/placeholder.svg?height=96&width=80" },
          },
          amount: 235266,
          date: "2025-08-07",
          cropInfo: {
          ...
          friendDetails: {
            ...
            imageUrl: "/placeholder.svg?height=96&width=80",
          },
        },
      ],
      settings: {
        organizationName: "Sample Cooperative Society",
      },
    },
  }
...
const processApiData = (jsonData: any) => {
  try {
    const mappedForms: LoanFormData[] = jsonData.kccData.members.map((member: any, index: number) => {
      const cropInfo = parseCrops(member.category)
      const approvalDate = member.date ? new Date(member.date).toLocaleDateString("en-GB") : ""
      const landInAcres = member.landArea ? (member.landArea / 2.471).toFixed(2) : ""
      ...
      const cropRows = member.selectedCrops?.map((crop: any, cropIndex: number) => {
        ...
        return {
          ...
          extentLand: crop.acres ? `${crop.acres}ac` : "",
      ...
      // If no selectedCrops, fall back to the original method
      if (cropRows.length === 0) {
        cropInfo.crops.forEach((crop, cropIndex) => {
          cropRows.push({
            slNo: (cropIndex + 1).toString(),
            cropName: crop.name,
            extentLand: crop.area,
            ...
            pesticide: "0",
            othersInsurance: "0",
            total: crop.amount || (member.amount ? member.amount.toString() : "235266"),
            date: approvalDate,
            description: "",
            credit: "0",
            disbursement: crop.amount || (member.amount ? member.amount.toString() : "235266"),
            balance: crop.amount || (member.amount ? member.amount.toString() : "235266"),
            noOfDays: "0",
            product: "0",
            interestDue: "0",
            interestReceived: "0",
      ...
      return {
        uEnn: member.userInformation?.உ_எண் || member.accountNo || 
              (hasValidFriendDetails ? friendDetails.uNumber || friendDetails.detailedInfo?.உ_எண் || "" : ""),
        name: member.userInformation?.பெயர் || member.memberName || "",
        fatherHusbandName: member.userInformation?.தகபெயர் || member.fatherName || 
                          (hasValidFriendDetails ? friendDetails.fatherName || friendDetails.detailedInfo?.தகபெயர் || "" : ""),
        address: member.userInformation?.முகவரி || 
                (hasValidFriendDetails ? friendDetails.address || friendDetails.detailedInfo?.முகவரி || "" : ""),
        ...
        aadhaarNo: member.userInformation?.ஆதார்_எண் || member.aadhaarNo || 
                  (hasValidFriendDetails ? friendDetails.aadhaarNumber || friendDetails.detailedInfo?.ஆதார்_எண் || "" : ""),
        ...
        mobileNo: member.userInformation?.கைபேசி_எண் || member.phoneNo || 
                 (hasValidFriendDetails ? friendDetails.phone || friendDetails.detailedInfo?.கைபேசி_எண் || "" : ""),
        ...
        guarantorUEnn:member.friendDetails?.uNumber||"" ,
        guarantorName: hasValidFriendDetails ? friendDetails.name || friendDetails.detailedInfo?.பெயர் || "" : "",
        guarantorAddress: hasValidFriendDetails ? friendDetails.address || friendDetails.detailedInfo?.முகவரி || "" : "",
...
const API_URL = process.env.NEXT_PUBLIC_API_URL;
  ...
  const calculateGrandTotal = (formIndex: number, column: string) => {
    return formsData[formIndex]?.tableRows
      .reduce((sum, row) => sum + Number.parseFloat(row[column as keyof typeof row] || "0"), 0)
      .toString() || "0"
  }
  ...
  const handleViewPdf = async () => {
    ...
    try {
      ...
      const result = await generatePDF(pages, { totalForms: formsData.length }, { autoDownload: false })
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="flex gap-4 mb-4">
        <Button onClick={handleViewPdf} disabled={isGeneratingPdf || isPdfMode}>
      ...
      {!pdfData && (
        ...
        <table className="w-full border-collapse" style={{ tableLayout: 'fixed' }}>
          <thead>
            <tr style={{ backgroundColor: "#f3f4f6" }}>
              <th rowSpan={2} className="border-2 border-black p-1 text-[9px] font-bold text-center align-middle" style={{ width: '25px' }}>
                Sl. No
              </th>
              <th rowSpan={2} className="border-2 border-black p-1 text-[9px] font-bold text-center align-middle" style={{ width: '60px' }}>
                Name of the Crop
              </th>
              <th rowSpan={2} className="border-2 border-black p-1 text-[9px] font-bold text-center align-middle" style={{ width: '45px' }}>
                Extent of Land Ac Cent
              </th>
              <th colSpan={6} className="border-2 border-black p-1 text-[9px] font-bold text-center">
                Expense Categories
              </th>
              <th rowSpan={2} className="border-2 border-black p-1 text-[9px] font-bold text-center align-middle" style={{ width: '40px' }}>
                Total
              </th>
              <th colSpan={5} className="border-2 border-black p-1 text-[9px] font-bold text-center">
                Account Details
              </th>
              <th rowSpan={2} className="border-2 border-black p-1 text-[9px] font-bold text-center align-middle" style={{ width: '30px' }}>
                No of Days
              </th>
              <th rowSpan={2} className="border-2 border-black p-1 text-[9px] font-bold text-center align-middle" style={{ width: '40px' }}>
                Product
              </th>
              <th colSpan={2} className="border-2 border-black p-1 text-[9px] font-bold text-center">
                Interest
              </th>
              <th rowSpan={2} className="border-2 border-black p-1 text-[9px] font-bold text-center align-middle" style={{ width: '30px' }}>
                Initial
            ...
            <tr style={{ backgroundColor: "#f3f4f6" }}>
              {/* Expense Categories Sub-headers */}
              <th className="border-2 border-black p-1 text-[9px] font-bold text-center" style={{ width: '30px' }}>Cash</th>
              <th className="border-2 border-black p-1 text-[9px] font-bold text-center" style={{ width: '40px' }}>Seed/ Raw Materials</th>
              <th className="border-2 border-black p-1 text-[9px] font-bold text-center" style={{ width: '35px' }}>Fertilizer</th>
              <th className="border-2 border-black p-1 text-[9px] font-bold text-center" style={{ width: '35px' }}>Micro Nutrients</th>
              <th className="border-2 border-black p-1 text-[9px] font-bold text-center" style={{ width: '35px' }}>Pesticide</th>
              <th className="border-2 border-black p-1 text-[9px] font-bold text-center" style={{ width: '40px' }}>Others/ Insurance</th>
              
              {/* Account Details Sub-headers */}
              <th className="border-2 border-black p-1 text-[9px] font-bold text-center" style={{ width: '30px' }}>தேதி</th>
              <th className="border-2 border-black p-1 text-[9px] font-bold text-center" style={{ width: '40px' }}>விபரம்</th>
              <th className="border-2 border-black p-1 text-[9px] font-bold text-center" style={{ width: '30px' }}>வரவு</th>
              <th className="border-2 border-black p-1 text-[9px] font-bold text-center" style={{ width: '40px' }}>பட்டுவாடா</th>
              <th className="border-2 border-black p-1 text-[9px] font-bold text-center" style={{ width: '30px' }}>நிலுவை</th>
              
              {/* Interest Sub-headers */}
              <th className="border-2 border-black p-1 text-[9px] font-bold text-center" style={{ width: '30px' }}>Due</th>
              <th className="border-2 border-black p-1 text-[9px] font-bold text-center" style={{ width: '35px' }}>Received</th>
            </tr>
          </thead>
          <tbody>
            {formData.tableRows.map((row, rowIndex) => (
              <tr key={rowIndex}>
                <td className="border border-black p-1 text-center text-[9px]">{row.slNo}</td>
                <td className="border border-black p-1 text-left text-[9px]">{row.cropName}</td>
                <td className="border border-black p-1 text-center text-[9px]">{row.extentLand}</td>
                <td className="border border-black p-1 text-right text-[9px]">{row.cash}</td>
                <td className="border border-black p-1 text-right text-[9px]">{row.seedRawMaterials}</td>
                <td className="border border-black p-1 text-right text-[9px]">{row.fertilizer}</td>
                <td className="border border-black p-1 text-right text-[9px]">{row.microNutrients}</td>
                <td className="border border-black p-1 text-right text-[9px]">{row.pesticide}</td>
                <td className="border border-black p-1 text-right text-[9px]">{row.othersInsurance}</td>
                <td className="border border-black p-1 text-right font-bold text-[9px]">{row.total}</td>
                <td className="border border-black p-1 text-center text-[9px]">{row.date}</td>
                <td className="border border-black p-1 text-left text-[9px]">{row.description}</td>
                <td className="border border-black p-1 text-right text-[9px]">{row.credit}</td>
                <td className="border border-black p-1 text-right text-[9px]">{row.disbursement}</td>
                <td className="border border-black p-1 text-right text-[9px]">{row.balance}</td>
                <td className="border border-black p-1 text-right text-[9px]">{row.noOfDays}</td>
                <td className="border border-black p-1 text-right text-[9px]">{row.product}</td>
                <td className="border border-black p-1 text-right text-[9px]">{row.interestDue}</td>
                <td className="border border-black p-1 text-right text-[9px]">{row.interestReceived}</td>
                <td className="border border-black p-1 text-right text-[9px]">{row.initial}</td>
              </tr>
            ))}
            {/* Total Row */}
            <tr>
              <td colSpan={2} className="border border-black p-1 text-center font-bold text-[9px]">
                மொத்தம்
              </td>
              ...
              <td className="border border-black p-1 text-right font-bold text-[9px]">
              ...
              <td className="border border-black p-1 text-right font-bold text-[9px]">
                {formData.tableRows.reduce((sum, row) => sum + (Number(row.fertilizer) || 0), 0)}
              </td>
              <td className="border border-black p-1 text-right font-bold text-[9px]">
                {formData.tableRows.reduce((sum, row) => sum + (Number(row.microNutrients) || 0), 0)}
              </td>
              <td className="border border-black p-1 text-right font-bold text-[9px]">
                {formData.tableRows.reduce((sum, row) => sum + (Number(row.pesticide) || 0), 0)}
              </td>
              <td className="border border-black p-1 text-right font-bold text-[9px]">
                {formData.tableRows.reduce((sum, row) => sum + (Number(row.othersInsurance) || 0), 0)}
              </td>
              <td className="border border-black p-1 text-right font-bold text-[9px]">
              ...
              <td className="border border-black p-1"></td>
              <td className="border border-black p-1 text-right font-bold text-[9px]">
                {formData.tableRows.reduce((sum, row) => sum + (Number(row.credit) || 0), 0)}
              </td>
              <td className="border border-black p-1 text-right font-bold text-[9px]">
              ...
              <td className="border border-black p-1 text-right font-bold text-[9px]">
                {formData.tableRows.reduce((sum, row) => sum + (Number(row.noOfDays) || 0), 0)}
              </td>
              <td className="border border-black p-1 text-right font-bold text-[9px]">
                {formData.tableRows.reduce((sum, row) => sum + (Number(row.product) || 0), 0)}
              </td>
              <td className="border border-black p-1 text-right font-bold text-[9px]">
                {formData.tableRows.reduce((sum, row) => sum + (Number(row.interestDue) || 0), 0)}
              </td>
              <td className="border border-black p-1 text-right font-bold text-[9px]">
                {formData.tableRows.reduce((sum, row) => sum + (Number(row.interestReceived) || 0), 0)}
              </td>
              <td className="border border-black p-1 text-right font-bold text-[9px]">
                {formData.tableRows.reduce((sum, row) => sum + (Number(row.initial) || 0), 0)}
              </td>
            </tr>
          </tbody>
        </table>
